...
def set_parser_arguments():
    ...
    parser.add_argument('-ts2', default="Oracle-Tags.CreatedOn", dest='tagspecial2', help='tag special key 2 to load the data to TAG_SPECIAL2 column')
    parser.add_argument('-ts3', default="SIAGCIS.Member", dest='tagspecial3', help='tag special key 3 to load the data to TAG_SPECIAL3 column')
    parser.add_argument('-ts4', default="SIAGCIS.TagTest", dest='tagspecial4', help='tag special key 4 to load the data to TAG_SPECIAL4 column')
    parser.add_argument('-d', default="", dest='filedate', help='Minimum File Date to load (i.e. yyyy-mm-dd)')
    parser.add_argument('-p', default="", dest='proxy', help='Set Proxy (i.e. www-proxy-server.com:80) ')
    parser.add_argument('-su', action='store_true', default=True, dest='skip_usage', help='Skip Load Usage Files')
...
def check_database_table_structure_usage(connection, tenant_name):
    try:
        ...
        with connection.cursor() as cursor:
            ...
            # check if TAG_SPECIAL3 columns exist in OCI_USAGE table, if not create
            sql = "select count(*) from user_tab_columns where table_name = 'OCI_USAGE' and column_name='TAG_SPECIAL3'"
            cursor.execute(sql)
            val, = cursor.fetchone()

            # if columns TAG_SPECIAL3 not exist, create it
            if val == 0:
                print("   Column TAG_SPECIAL3 does not exist in the table OCI_USAGE, adding...")
                sql = "alter table OCI_USAGE add (TAG_SPECIAL3 VARCHAR2(4000))"
                cursor.execute(sql)
                
            # check if TAG_SPECIAL4 columns exist in OCI_USAGE table, if not create
            sql = "select count(*) from user_tab_columns where table_name = 'OCI_USAGE' and column_name='TAG_SPECIAL4'"
            cursor.execute(sql)
            val, = cursor.fetchone()

            # if columns TAG_SPECIAL4 not exist, create it
            if val == 0:
                print("   Column TAG_SPECIAL4 does not exist in the table OCI_USAGE, adding...")
                sql = "alter table OCI_USAGE add (TAG_SPECIAL4 VARCHAR2(4000))"
                cursor.execute(sql)
    ...
    try:
        ...
        with connection.cursor() as cursor:
            ...
            if tag_special_key3:
                ...
                start_time = time.time()
                # run merge to OCI_COST_REFERENCE for the tag special key
                print("   Handling Tag Special Key '" + tag_special_key3 + "'")
            ...
            if tag_special_key4:
                ...
                start_time = time.time()
                # run merge to OCI_COST_REFERENCE for the tag special key
                print("   Handling Tag Special Key '" + tag_special_key4 + "'")
        ...
        with connection.cursor() as cursor:
            ...
            (
                ...
                (
                    ...
                    union all
                    select /*+ parallel(OCI_USAGE,8) full(OCI_USAGE) */ distinct TENANT_NAME, 'USAGE_TAG_SPECIAL3' as REF_TYPE, TAG_SPECIAL3 as ref_name from OCI_USAGE  where :tenant_name = TENANT_NAME
                    union all
                    select /*+ parallel(OCI_USAGE,8) full(OCI_USAGE) */ distinct TENANT_NAME, 'USAGE_TAG_SPECIAL4' as REF_TYPE, TAG_SPECIAL4 as ref_name from OCI_USAGE  where :tenant_name = TENANT_NAME
                ) where ref_name is not null
            ) b
            on (a.TENANT_NAME=b.TENANT_NAME and a.REF_TYPE=b.REF_TYPE and a.REF_NAME=b.REF_NAME)
            when not matched then insert (TENANT_NAME,REF_TYPE,REF_NAME)
...
def load_usage_file(connection, object_storage, object_file, max_file_id, cmd, tenancy, compartments, file_num, total_files, costusage_namespace_name, costusage_bucket_name):
    ...
    try:
        ...
        with gzip.open(path_filename, 'rt') as file_in:
            ...
            USG_BILLED_QUANTITY, USG_CONSUMED_QUANTITY, USG_CONSUMED_UNITS, USG_CONSUMED_MEASURE, IS_CORRECTION, TAGS_DATA, TENANT_ID, TAG_SPECIAL, TAG_SPECIAL2, TAG_SPECIAL3, TAG_SPECIAL4
            ) VALUES (
...
def main_process():
    ...
    ############################################
    #Change this for customer
    #costusage_bucket_name = ""
    #costusage_namespace_name = cmd.namespace_name
    costusage_bucket_name = "internal-ocid1.tenancy.oc1..aaaaaaaajf4cprju24l6tsgsamcy42oqdhocpbk3myc3vwtzb7qx7lizsu4q"
    ...
    try:
        ...
        with oracledb.connect(user=cmd.duser, password=dbpass, dsn=cmd.dname) as connection:
            ...
            if usage_num > 0 or cmd.force:
                ...
                update_usage_reference(connection, cmd.tagspecial, cmd.tagspecial2, cmd.tagspecial3, cmd.tagspecial4, tenancy.name)
